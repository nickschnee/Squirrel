...
This is a generalized linear model (GLM) with a Poisson family that aims to predict the number of squirrels based on the date and whether the shift is PM or not. The coefficients of the model are estimated using the maximum likelihood method.

The model shows that the number of squirrels decreases as the date increases, while the shift being PM has a positive effect on the number of squirrels. In other words, the model suggests that there are fewer squirrels as time goes by, and that there are more squirrels in the afternoon shift compared to the morning shift.

The significance codes indicate that the intercept, the date, and the shift are all statistically significant predictors of the number of squirrels. The null deviance and residual deviance are measures of the goodness of fit of the model. The AIC provides a measure of the quality of the model relative to other models.

Overall, this GLM suggests that the date and the shift are important predictors of the number of squirrels, and the model fits the data well.

## Overview over the Number of Occurrences of the Different Weather Conditions (and Other Things)
...
The model is a Poisson regression that predicts the number of squirrels seen based on various weather conditions. The variables included in the model are: sunny, drizzly, cloudy, foggy, rainy, humid, Shift (morning or afternoon), Hectare.Conditions (categorical variable with levels: "Calm", "Busy", "Moderate", "Calm, Busy", and "Medium"), and temperature_celsius.

The model shows that rainy weather conditions are associated with fewer squirrels seen, while the other weather conditions (sunny, drizzly, cloudy, foggy, and humid) do not have a statistically significant effect on the number of squirrels seen. Additionally, an increase in temperature by one degree Celsius is associated with an increase in the expected number of squirrels by 1.7%.

The model also shows that being in the afternoon shift is associated with more squirrels seen compared to the morning shift. The Hectare.Conditions variable does not have a statistically significant effect on the number of squirrels seen.

Overall, the model suggests that temperature and time of day may be the most important factors in predicting the number of squirrels seen. However, it is important to note that the model's predictions may not be accurate for weather conditions and environments outside of the observed data.

## Regression with Time and Temperature (with Interaction Term)
